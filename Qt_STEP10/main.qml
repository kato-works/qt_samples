...
Window {
    ...
    width: 640
    height: 480
    title: qsTr("Qt STEP10")

    signal sendCanMessage(string frameId, string frameMsg)

    Component.onCompleted: {
        sendCanMessage.connect(controller001.sendCanMessage);
        model001.messageReceived.connect(canModel.updateMessage);
    }
    ...
    Item {
        ...
        function updateMessage(message){
            console.log("[canModel] updateMessage", message);
            canModel.receiveLog = message + "\n" + canModel.receiveLog;
        }
    }
